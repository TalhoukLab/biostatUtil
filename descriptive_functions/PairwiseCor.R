PairwiseCor <- function(dataframe){
  ...
  #
  # Args:
  #  dataframe:A data frame containing numeric variables of interest. 
  #
  # Returns:
  #  pairwiseCorDF:  all pairwise absolute correlations,  correlations, Pval, Adj P val by decreasing order of absolute corre,ations.
 ...
 df <- data.frame(Vairable1=rep(0,length(pairs)), Variable2=rep(0,length(pairs)), 
                   AbsCor=rep(0,length(pairs)), Cor=rep(0,length(pairs)),Pval=rep(0,length(pairs)),AdjP=rep(0,length(pairs)))
