...
############################
# bootstrap kappa
# by Aline Talhouk (2015-02-24)
#
kappaCIboot<-function(x,y,seed=20,num.boot=1000){
	set.seed(seed)
	require(boot)
	require(psy)
	#Function to compute cohen's kappa with binary data and 95% CI based on 1000 bootstrap samples
	#to obtain a 95%confidence interval:
	ckappa.boot <- function(data,x) {ckappa(data[x,])[[2]]}
	res <- boot(cbind(x,y),ckappa.boot,num.boot)
	quantile(res$t,c(0.025,0.975),na.rm=T)# two-sided bootstrapped confidence interval of kappa
	bootCI=(boot.ci(res,type="bca"))# adjusted bootstrap percentile (BCa) confidence interval (better)
	kappa=c(bootCI[[2]],bootCI[[4]][,4:5])
	return(kappa)
}

############################
# similar to confusion matrix but with confidence interval
# works for 2 categories
# print out confidence interval as well
#
# by Aline Talhouk (2015-02-24)
#
confusionMatrixTwoCatWithCI=function(x,y,seed=20,num.boot=1000){
	require(Hmisc)
	...
	n2=(CM[2,1]+CM[2,2])
	PO=(CM[1,1]+CM[2,2])
	Accuracy=round(binconf(PO,m1+m2,method="wilson"),4)
	Sensitivity=round(binconf(CM[1,1],m1,method="wilson"),4)
	Specificity=round(binconf(CM[2,2],m2,method="wilson"),4)
	PPV=round(binconf(CM[1,1],n1,method="wilson"),4)
	NPV=round(binconf(CM[2,2],n2,method="wilson"),4)
	kappa=round(kappaCIboot(x,y,seed,num.boot),4)
	printCI<-function(z){paste(z[1],"(",z[2],"-",z[3],")")}
	print(paste("Accuracy :", printCI(Accuracy)))
	print(paste("Sensitivity :", printCI(Sensitivity)))
	print(paste("Specificity :", printCI(Specificity)))
	print(paste("PPV :", printCI(PPV)))
	print(paste("NPV :", printCI(NPV)))
	print(paste("kappa :", printCI(kappa)))
	return(list(Sensitivity=Sensitivity,Specificity=Specificity,PPV=PPV,NPV=NPV,kappa=kappa))
}
	...
	if (pretty.text) {
		...
		return(
			apply(
				...
				function(x){
					if (!is.nan(x)) {
						return(paste(format(x,digits=digits),"%",sep=""))
					} else {
						return("-")
					}
				}
...
####################################################
# same as row.percent but show table in html
...
####################################################
# same as single.row.percent but show table in html
single.row.percent.as.html <- function(
		t, 
		show.count=FALSE, # whether to show the count
		digits=4,
		column.names=NULL,
	...
	col.th.style <- COL.TH.STYLE
	row.th.style <- ROW.TH.STYLE
	table.values <- single.row.percent(t, pretty.text=TRUE, digits=digits)
	# header
	if (!is.null(column.names)) {
		colnames(table.values) <- column.names
	} else {
...
col.percent <- function(t, pretty.text=FALSE, digits=4) {
	if (pretty.text) {
		return(
			apply(
				t(t(t)/apply(t,2,sum))*100, 
				c(1,2),
				function(x){
					if (!is.nan(x)) {
						return(paste(format(x,digits=digits),"%",sep=""))
...
#################################################################
# generate a table with count, row %, column % given the table t
# i.e. the return table will have row = nrow(t)*3
row.col.percent <- function(t,...) {
	row.p <- row.percent(t,...);
	col.p <- col.percent(t,...);
	
	result <- rep(NA, nrow(t)*3*ncol(t))
	dim(result) <- c(nrow(t)*3, ncol(t))
	
	for (i in 1:nrow(t)) {
		for (j in 1:ncol(t)) {
			result[1+(i-1)*3,j] <- t[i,j]
			result[1+(i-1)*3+1,j] <- row.p[i,j]
			result[1+(i-1)*3+2,j] <- col.p[i,j]			
		}
	}
	
	colnames(result) <- colnames(t)
	result.rownames <- rep(NA, nrow(t)*3)
	for (i in 1:nrow(t)) {
		result.rownames[1+(i-1)*3] <- rownames(t)[i]
		result.rownames[1+(i-1)*3+1] <- "row percent"
		result.rownames[1+(i-1)*3+2] <- "col percent"
	}
					...
					paste(
						...
						ifelse(
							...
							ifelse(show.missing,1+sum(!is.na(missing.codes.highlight[[var.name]])),0)+1+as.numeric(ifelse(
								...
								length(names(table(droplevels(input.d.no.missing.var.only[!is.na(input.d.no.missing.var.only[,var.name]) & !input.d.no.missing.var.only[,var.name]%in%missing.codes,var.name])))),
								length(names(table(           input.d.no.missing.var.only[!is.na(input.d.no.missing.var.only[,var.name]) & !input.d.no.missing.var.only[,var.name]%in%missing.codes,var.name] )))
							))
						),
