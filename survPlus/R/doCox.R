#' A function to perform Cox proportional hazard with or without firth
#' correction
#'
...
doCox <- function(formula, data, firth = TRUE) {
  if (!is.data.frame(data))
    stop("Must provide data in a data.frame")
  # Decompose the model
  ...
  if (firth) {
    ...
    res <- cbind(beta.lp = fit$coefficients,
                 lower = log(fit$ci.lower), upper = log(fit$ci.upper))
