...
######################################################
# need to calibrate xlsx in case trying to read excel
if (exists("read.xlsx2")) {
	calibrator.d <- read.xlsx2("xlsx_calibrator.xlsx",1,stringsAsFactors=FALSE)
	EXCEL.BASE.DATE <- as.Date(calibrator.d$excel_text[1])-as.numeric(calibrator.d$excel_date[1])
	for (i in 2:nrow(calibrator.d)) {
		if (difftime(EXCEL.BASE.DATE, as.Date(calibrator.d$excel_text[i])-as.numeric(calibrator.d$excel_date[i]))!=0) {
			cat("ERROR: xlsx calibration FAILED ... please check date_function.R.  You may have error when trying to read excel file with dates.\n")
		}
	}
}
#
...
#
min.date <- function(dates, na.rm=TRUE) {
	if(na.rm) {
		dates <- dates[!is.na(dates)]
	}
	if (sum(is.na(dates)) >= 1) {return(NA)} # there is NA
...
################
# numeric to date with default origin of 1970-01-01
#
numeric.to.date <- function(x) {
...
# format date string
#
# INCOMPLETE!!! not all format included
format.date.string.from.day.month.year <- function(day,month,year,date.format) {
	if (date.format==MM.DD.YYYY) {
		return(paste(month,day,year,sep="/"))
	} else if (date.format==DD.MM.YYYY) {
		return(paste(day,month,year,sep="/"))
	} else if (date.format==YYYY_MM_DD) {
		return(paste(year,month,day,sep="-"))
	} else {
		cat("ERROR (format.date.string.from.day.month.year): unknown format: ",date.format,"\n",sep="")
		return(NA)
	}
}
