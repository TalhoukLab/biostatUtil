...
item14.correlation.table <- doCohortCharacteristics(emdb, "POLE.mut.germline.as.missing", "POLE", 
	...
	stat.tests=c("spearman","spearman","kendall","kendall","chisq","kendall","kendall","chisq"),
...
logit.ex.bmi.d$stage_b1v234 <- sapply(logit.ex.bmi.d$stage_b1v234,as.character); logit.ex.bmi.d$stage_b1v234 <- factor(logit.ex.bmi.d$stage_b1v234,levels=names(table(logit.ex.bmi.d$stage_b1v234)))
logit.ex.bmi.d$LVSI <- sapply(logit.ex.bmi.d$LVSI,as.character); logit.ex.bmi.d$LVSI <- factor(logit.ex.bmi.d$LVSI,levels=names(table(logit.ex.bmi.d$LVSI)))
logit.ex.bmi.d$any.positive.nodes <- sapply(logit.ex.bmi.d$any.positive.nodes,as.character); logit.ex.bmi.d$any.positive.nodes <- factor(logit.ex.bmi.d$any.positive.nodes,levels=names(table(logit.ex.bmi.d$any.positive.nodes)))
