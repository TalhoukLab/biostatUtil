...
item12.tma.breakdown <- doCohortCharacteristics(temp.d, "POLE.mut.germline.as.missing", "POLE", 
		...
		c(FALSE), 
		c("tissue microarray"),
		caption="Distribution of POLE wild type / mutated case among TMA's (includes cases represented in multiple TMA's)",
		show.missing=FALSE,
		banded.rows=FALSE
)
...
if (!RUN.IN.MARKDOWN) {
	print(emdb.w.duplicate[emdb.w.duplicate$Study.Identifier %in% emdb.w.duplicate$Study.Identifier[emdb.w.duplicate$comment.about.duplicate=="first occurrence"],c("TMA","comment.about.duplicate")])
}