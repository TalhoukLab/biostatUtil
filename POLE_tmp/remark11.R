# POLE paper - Remark #13 (how marker values were handled)
# 
# Author: samuelc
###############################################################################
...
# 10-005 and 10-006
temp.d <- emdb.w.duplicate[emdb.w.duplicate$comment.about.duplicate == "" |  emdb.w.duplicate$comment.about.duplicate != "first occurrence",]
...
pole.mut.amino.acid.changes <- sapply(temp.d$POLE.mut,function(x){
	...
	if (x %in% c("","Wild Type")) {
		...
		return(trim.white.spaces(strsplit(strsplit(x,"\\(")[[1]][1]," ")[[1]][1]))				
	}
})
