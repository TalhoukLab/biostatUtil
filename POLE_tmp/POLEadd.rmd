...
doTrtTrendBarplot(
  ...
  title = paste0("Adjuvant treatment, whole cohort\n(",
                sum(emdb$init.treatment%in%ALL.MISSING.CODES),
...
doTrtTrendBarplot(
  ...
  title = paste0("Adjuvant treatment, among POLE mutated\n(",
                sum(emdb$init.treatment[emdb$POLE.mut.germline.as.missing == VALUE.CODING.MUTATED] %in% ALL.MISSING.CODES),
...
doSurvTrendBarplot(
  ...
  paste0(obs.yrs, "-years overall survival, among POLE mutated\n(no missing data)"), 
  "os.yrs", 
  "os.sts", 
  "os.event", 
  "dead", 
...
doSurvTrendBarplot(
  ...
  paste0(obs.yrs, "-years disease specific survival, among POLE mutated\n(",
        sum(is.na(emdb$dss.yrs[emdb$POLE.mut.germline.as.missing == VALUE.CODING.MUTATED])),
        " case with missing data)"), 
  "dss.yrs", 
  "dss.sts", 
  "dss.event", 
...
doSurvTrendBarplot(
  ...
  paste0(obs.yrs,"-years relapse free survival, among POLE mutated\n(",
        sum(is.na(emdb$rfs.yrs[emdb$POLE.mut.germline.as.missing == VALUE.CODING.MUTATED])),
        " cases with missing data)"), 
  "rfs.yrs", 
  "rfs.sts", 
  "rfs.event", 
...
doGrdTrendBarplot(
  emdb[emdb$POLE.mut.germline.as.missing == VALUE.CODING.MUTATED, ],
  title = paste0("Grade, among POLE mutated\n(no missing data)"),
  legend.space = 0.2)
```
...
```{r}
logistfPrint(logit.full.ex.bmi.firth)
```
...
pole_vs_stage_x_age <- doCohortCharacteristics(emdb, "POLE.mut.germline.as.missing", "POLE", 
  	c("stage_x_age"), 
		c(FALSE), 
		c("Stage x age"),
		stat.tests=c("fisher"),
...
pole_vs_stage_x_bmi <- doCohortCharacteristics(emdb, "POLE.mut.germline.as.missing", "POLE", 
    c("stage_x_bmi"), 
		c(FALSE), 
		c("Stage x BMI"),
		stat.tests=c("fisher"),
...
pole_vs_age_x_bmi <- doCohortCharacteristics(emdb, "POLE.mut.germline.as.missing", "POLE", 
    c("age_x_bmi"), 
  	c(FALSE), 
		c("Age x BMI"),
		stat.tests=c("fisher"),
...
pole_vs_stage_x_age_x_bmi <- doCohortCharacteristics(emdb, "POLE.mut.germline.as.missing", "POLE", 
    c("stage_x_age_x_bmi"), 
  	c(FALSE), 
		c("Stage x age x BMI"),
		stat.tests=c("fisher"),
...
**Analysis among grade 3 - univariable**

```{r fig.keep='none'}
source("remarks/remark15.R")
```
...
# width/height = 13/5
  doKMPlots(
		...
		var.description = "POLE mutation status - grade 3",
		single.test.type = "logrank",
		time = "os.yrs",
		status = "os.sts == 'os.event'",
		xlabs = "Total follow-up (years)",
  ...
  doKMPlots(
    ...
    var.description = "POLE mutation status - grade 3",
		single.test.type = "logrank",
		time = "dss.yrs",
		status = "dss.sts == 'dss.event'",
		xlabs = "Total follow-up (years)",
    ylabs = "Disease specific survival (DSS)",
		surv.type = "dss",
		conf.int = TRUE,
		use.aline.plot = TRUE
	)
  doKMPlots(
  	emdb[!emdb$POLE.mut.germline.as.missing %in% ALL.MISSING.CODES &
  	       emdb$Tumour.Grade == "Grade 3" &
  	       !is.na(emdb$rfs.sts) & !is.na(emdb$rfs.yrs), ],
		var.name = "POLE.mut.germline.as.missing", 
		var.description = "POLE mutation status - grade 3",
		single.test.type = "logrank",
		time = "rfs.yrs",
		status = "rfs.sts == 'rfs.event'",
		xlabs = "Any relapse-free survival time",
    ylabs = "Any relapse-free survival",
		surv.type = "pfs",
		conf.int = TRUE,
		use.aline.plot = TRUE
	)
...
uni.cox.pole.g3 <- doCoxphGeneric( # a table of univariable cox models of treatment vs. no treatment
...
**Analysis among grade 3 - multivariable**

*Summary*: **when taking into account clinicopathological parameters in a multivariable Cox model,  POLE does not appear to change the hazard associated with OS and PFS, there is only evidence that POLE is protective of DSS.**
