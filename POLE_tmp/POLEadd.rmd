...
uni.cox.tx <- do.coxph.generic( # a table of univariable cox models of treatment vs. no treatment
  	emdb, 
		c("any.init.treatment"), 
		c(HTML.COX.VAR.DESC.ANY.INIT.ADJ.TX),
		use.firth=FIRTH.THRESHOLD,
		stat.test="logtest",
		var.ref.groups=c(NA),
		caption="Univariable Cox model of treatment vs. no treatment (OS/DSS/RFS) in whole cohort",
		banded.rows=TRUE)
uni.cox.tx.pole.wt <- do.coxph.generic( # a table of univariable cox models of treatment vs. no treatment
    emdb[emdb$POLE.mut.germline.as.missing=="wild type",], 
		c("any.init.treatment"), 
		c(HTML.COX.VAR.DESC.ANY.INIT.ADJ.TX),
		use.firth=FIRTH.THRESHOLD,
		stat.test="logtest",
		var.ref.groups=c(NA),
		caption="Univariable Cox model of treatment vs. no treatment (OS/DSS/RFS) among POLE wild type",
		banded.rows=TRUE)
uni.cox.tx.pole.mut <- do.coxph.generic( # a table of univariable cox models of treatment vs. no treatment
    emdb[emdb$POLE.mut.germline.as.missing=="mutated",], 
  	c("any.init.treatment"), 
		c(HTML.COX.VAR.DESC.ANY.INIT.ADJ.TX),
		use.firth=-1,
		stat.test="logtest",
		var.ref.groups=c(NA),
		caption="Univariable Cox model of treatment vs. no treatment (OS/DSS/RFS) among POLE mutated",
		banded.rows=TRUE)
# per meeting with Aline 2015-05-29, no need to show this any more.
```

`r #uni.cox.tx$result.table.html`
`r #uni.cox.tx.pole.wt$result.table.html`
`r #uni.cox.tx.pole.mut$result.table.html`
