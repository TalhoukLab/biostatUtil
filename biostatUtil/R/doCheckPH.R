...
doCheckPH <- function(var.name, var.description) {
  ...
  test <- pretty.coxph(as.formula(paste("Surv(os.yrs, os.sts=='os.event'  ) ~",
                                        var.name)),
                       input.d = temp.d,
                       check.ph = TRUE,
                       ph.test.plot.filename = NA)
  ...
  test <- pretty.coxph(as.formula(paste("Surv(dss.yrs, dss.sts=='dss.event'  ) ~",
                                        var.name)),
  ...
  test <- pretty.coxph(as.formula(paste("Surv(rfs.yrs, rfs.sts=='rfs.event'  ) ~",
                                        var.name)),
