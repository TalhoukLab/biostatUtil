...
testInter<-function(dmat, event, labs = c("POLE wt", "Rx", "POLE wt * Rx"),
  ...
  if (use.firth) {
    switch(event,
           OS = {
             cox1=coxphf::coxphf(Surv(OS.yrs,OS.sts=="os.event")~POLE,data=dmat)
             cox2=coxphf::coxphf(Surv(OS.yrs,OS.sts=="os.event")~POLE+Rx,data=dmat)
             cox3=coxphf::coxphf(Surv(OS.yrs,OS.sts=="os.event")~POLE*Rx,data=dmat)
           },
           DSS = {
             cox1=coxphf::coxphf(Surv(DSS.yrs,DSS.sts=="dss.event")~POLE,data=dmat)         
             cox2=coxphf::coxphf(Surv(DSS.yrs,DSS.sts=="dss.event")~POLE+Rx,data=dmat)
             cox3=coxphf::coxphf(Surv(DSS.yrs,DSS.sts=="dss.event")~POLE*Rx,data=dmat)  		
           },
           RFS = {
             cox1=coxphf::coxphf(Surv(RFS.yrs,RFS.sts=="rfs.event")~POLE,data=dmat)
             cox2=coxphf::coxphf(Surv(RFS.yrs,RFS.sts=="rfs.event")~POLE+Rx,data=dmat)
             cox3=coxphf::coxphf(Surv(RFS.yrs,RFS.sts=="rfs.event")~POLE*Rx,data=dmat)         
           }
    )
  } else {
    switch(event,
  ...
  if (use.firth) {
    lrt1.p <- coxphf::coxphftest(cox2$formula, test = ~POLE, data = dmat)$prob
    lrt2.p <- coxphf::coxphftest(cox3$formula, test = ~POLE + Rx, data = dmat)$prob
  } else {
