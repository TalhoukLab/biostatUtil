...
test_that("factor with n levels can be separated into at most n groups", {
  expect_error(best_cut(Surv(time, status) ~ fac, lung_mod, n = "b", plot = FALSE), NA)
  expect_error(best_cut(Surv(time, status) ~ fac, lung_mod, n = "t", plot = FALSE), NA)
  expect_error(best_cut(Surv(time, status) ~ fac, lung_mod, n = "qd", plot = FALSE), NA)
  expect_error(best_cut(Surv(time, status) ~ fac, lung_mod, n = "qn", plot = FALSE))
})
...
test_that("cutpoint criteria changes when likelihood is flat", {
  expect_error(best_cut(Surv(time, status) ~ fac, lung_mod, n = "b", plot = FALSE,
                        AIC.range = 1e-2), NA)
})
