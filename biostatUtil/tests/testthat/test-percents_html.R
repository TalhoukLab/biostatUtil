...
test_that("Other formatting options work", {
  ...
  expect_error(htmlTable(rowColPercentAsHTML(B, keep = FALSE, banded.rows = TRUE,
                                             col.odd = "yellow", col.even = "green")), NA)
})

test_that("Row and column names can be overwritten", {
  expect_error(htmlTable(colPercentAsHTML(B, keep = TRUE, row.names = letters[1:5],
                                          column.names = LETTERS[1:4])), NA)
  expect_error(htmlTable(rowPercentAsHTML(B, keep = TRUE, row.names = letters[1:5],
                                          column.names = LETTERS[1:4])), NA)
  expect_error(htmlTable(rowColPercentAsHTML(B, keep = TRUE, row.names = letters[1:5],
                                             column.names = LETTERS[1:4])), NA)
})
