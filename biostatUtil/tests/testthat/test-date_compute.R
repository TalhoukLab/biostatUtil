...
test_that("Adding date works for allowed units", {
  expect_identical("1992/02/06", addToDate(x5, 10, date.format = "YYYY.MM.DD", units = "days"))
  expect_identical("1992/04/06", addToDate(x5, 10, date.format = "YYYY.MM.DD", units = "weeks"))
  expect_identical("1992/11/27", addToDate(x5, 10, date.format = "YYYY.MM.DD", units = "months"))
  expect_identical("2002/01/26", addToDate(x5, 10, date.format = "YYYY.MM.DD", units = "years"))
})

test_that("Adding date handles erroneous inputs", {
  expect_equal(NA, addToDate(NA, 10))
  expect_equal(NA, addToDate(x5, NA))
  expect_equal("Missing", addToDate(x5, 10, existing.missing.codes = x5, return.missing.code = "Missing"))
})
