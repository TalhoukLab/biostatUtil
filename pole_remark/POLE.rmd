...
ALINE <- FALSE # if run on Aline's computer
RUN.IN.MARKDOWN=TRUE # a flag to indicate that the codes are called from this markdown file
if (ALINE) {
  LIB.DIR <- "~/Desktop/Rcode/R_scripts/"
  SETUP_KNITR.R <- "~/Desktop/Rcode/SetUp_knitr.R"
  DEFINEEVENTDATE.R <- "~/Desktop/Rcode/DefineEventDate.R"
  ED.CSV <- "~/Documents/Endometrial/Database/ed.csv"
  SETUP_DB.R <- "~/Documents/Endometrial/POLE/SetUp_db.R"
  DATE.FORMAT="%Y-%m-%d" # this should be R's default date format
  CASE.SELECTION.FLOW.CHART <- "/Users/atalhouk/Documents/Endometrial/POLE/remark.png"
  DO.CITATIONS=TRUE
} else {
  options(encoding = "UTF-8")
  LIB.DIR <- "C:\\Users\\samuelc\\Documents\\workspace\\R\\R_scripts\\"
  SETUP_KNITR.R <- "SetUp_knitr.R"
  DEFINEEVENTDATE.R <- "C:\\Users\\samuelc\\Documents\\workspace\\R\\endometrial\\jessica_2014-04-08\\DefineEventDate.R"
  ED.CSV <- "R:\\data\\analyses\\endometrial\\jessica_2014-04-08\\excel exports\\endometrial_n518_2015-05-12.csv"
  SETUP_DB.R <- "SetUp_db.R"
  DATE.FORMAT="%m/%d/%Y" # this should be R's default date format
  CASE.SELECTION.FLOW.CHART <- "R:\\data\\analyses\\endometrial\\paper_pole\\remark.png" # CANNOT TAKE SPACE IN FILE PATH!!!!
  DO.CITATIONS=FALSE
}
...
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=13, fig.height=5}
par(mfrow=c(1,2))
do.km.plots.pole("os")
```
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=8, fig.height=6}
do.km.plots.pole.x.init.treatment("os")
```

**KM plots - disease specific survival**
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=13, fig.height=5}
par(mfrow=c(1,2))
do.km.plots.pole("dss")
```
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=8, fig.height=6}
do.km.plots.pole.x.init.treatment("dss")
```

**KM plots - relapse free survival**
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=13, fig.height=5}
par(mfrow=c(1,2))
do.km.plots.pole("rfs")
```
```{r message=FALSE,echo=FALSE,results='hide',fig.keep='all', fig.width=8, fig.height=6}
do.km.plots.pole.x.init.treatment("rfs")
```
...
taos=testInter(dat1,event="OS",use.firth=TRUE)
...
Moreover, by accounting for treatment, there is a statistcially significant improvement to the likelihood Pvalue: `r round(taos[[4]]["P(>|Chi|)"][2,],4)`. The final step is to try to assess the statistical significance of adding an interaction term to the model (model 3). This appears to not be statistically signaficant, p value `r round(taos[[5]]["P(>|Chi|)"][2,],3)`. In conclusion, treatment is confounded with POLE but does not interact with POLE is as far as predicting time to death.
...
For disease specific survival, we are not able to properly assess the interaction effect, because the mutated, no treatment group has no events. However a similar trend as seen in the time to death analysis hold. The confounding effect of treatment is significant (LRT P-Value: `r round(taos[[4]]["P(>|Chi|)"][2,],4)`).
...
For relapse-free survival, we are not able to properly assess the interaction effect either, because the mutated, no treatment group has no events. However a similar trend as seen in the time to death analysis hold. The confounding effect of treatment is significant (LRT P-Value: `r round(taos[[4]]["P(>|Chi|)"][2,],4)`).
...
par(mfrow=c(1,3))
do.check.ph("POLE.mut.germline.as.missing","POLE mutation")
```
