...
A published dataset from TCGA uses `r nrow(hgsc)` genes to cluster `r ncol(hgsc) - 1` HGSC samples into the four subtypes: C1, C2, C4, and C5. They used consensus clustering with NMF. In this report, we consider the following clustering algorithms to use in consensus clustering:
...
Cohen's kappa statistic $\kappa$ measures the level of agreement between two raters. In our case, the raters are different clustering algorithms.
\begin{center}
$\kappa = \dfrac{p_o - p_e}{1 - p_e}$
\end{center}

The weighted $\kappa$ is as follows:
\begin{center}
$\kappa_w = 1 - \dfrac{\sum_{i=1}^k\sum_{j=1}^k w_{ij}x_{ij}}{\sum_{i=1}^k\sum_{j=1}^k w_{ij}m_{ij}}$
\end{center}

Fleiss gave the following interpretations for $\kappa$, albeit arbitrary and not numerically determined.
