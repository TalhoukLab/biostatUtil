...
```{r CDF}
CDF.each <- ldply(results.CCP, function(x)
  x[[4]]$consensusMatrix[lower.tri(x[[4]]$consensusMatrix)]) %>% 
  rbind(., ldply(list(results.NMF$nmfDiv$consensusMatrix,
                      results.NMF$nmfEucl$consensusMatrix),
...
pandoc.table(table(results.all[, 2]))
```
...
```{r weighted_meta_cm}
weights <- PAC.CHI.only[order(match(PAC.CHI.only$Algorithms, colnames(results.all))), "Weight"]
weighted.meta.cm <- consensusMatrix(results.all, weights)
