...
From the heatmap, we do not see a very high concordance across algorithms. For example, the proportion of cases with at least 0.6 agreement is only `r round(((sum(meta.cm >= 0.6) - ncol(meta.cm)) / 2) / choose(ncol(meta.cm), 2), 3)`. There is some evidence of a four-class data structure.
...
## External Evaluation
External evaluation refers to the situation when we compare our clustering assignments to true class labels, gold standards, or reference labels. In the TCGA analysis, we use their published clustering result as reference labels. The downside of using external evaluation is that the reference classes may not be correctly clustered themselves, and we are treating these as the norm. None the less, we can explore a few metrics to see how our own clustering performance compares.

We expect that our own NMF-based algorithms will perform well on these evaluation indices because the reference classes were clustered using NMF too.
...
The weighted $\kappa$ statistic is as follows and takes into account the off-diagonal elements of the confusion matrix between the two raters^6^. In other words, the disagreements between the raters are weighed in the calculation:
\begin{center}
...
Fleiss provided the following rating scheme for $\kappa$^7^. Albeit arbitrary and not numerically determined they still serve as a rough guideline.
...
kappas <- alply(tcga.all, 2, function(x)
  ...
  dplyr::select(Algorithm, kappa, weighted.kappa, Rating)
...
The Rand Index measures agreement between two classes by counting the number of pairs of objects that are clustered together in two different clustering assignments^8^. Hubert & Araibe propose an adjusted version to account for random chance^9^. The equation for the adjusted Rand index (ARI) is shown below^8,9,10^:
...
## Internal Evaluation
Interval evaluation assesses how well clustered objects are based on the data features they are measured on. Ideally, we want objects in the same cluster to be close together and objects in different clusters to be far from each other. Different definitions of distance define compactness and separation differently. Some evaluation indices combine these distances into a unitless measure we can use to compare clustering results.

### Proportion of Ambiguously Clustered Pairs (PAC)

The notion of PAC is closely related to consensus clustering. Senbabaoglu et al. argue that PAC is a better measure of determining the optimal number of clusters from consensus matrices, compared to other measures such as the Gap statistic, CLEST, etc^11^. The idea is simple: the proportion of ambiguously clustered pairs is the number of entries in the consensus matrix that are not 0 or 1. Recall that the consensus matrix is symmetric, so we only consider the lower (or upper) triangular matrix. In most applications, the definition of ambiguity is less stringent, and any entry greater than $p$ or less than $1 - p$ contributes to PAC, where $p$ is a small proportion (e.g. 0.05).
...
PAC <- tcga.CCP %>%
  ...
  c(nmfDiv = PAC(tcga.NMF$nmfDiv$consensusMatrix),
    nmfEucl = PAC(tcga.NMF$nmfEucl$consensusMatrix)) %>%
  set_names(ALG.NAMES) %>% 
  sort %>% 
  data.frame(PAC = .) %>% 
  mutate(Algorithms = rownames(.)) %>% 
...
### PAC and CHI only
There is a major issue with the way we ranked the indices in the previous section. Upon closer inspection, the algorithm that uses hierarchical clustering with single linkage and Euclidean distance actually performs terrible in the clustering analysis. The table below shows the number of samples clustered into each of the four clusters:

```{r hcSEucl_bad, results='asis'}
pandoc.table(table(tcga.all[, 2]))
```

And yet, the algorithm performs very well for several indices. One suggestion is to consider only PAC and CHI, because they do not depend on a specific distance metric. Also, the weights will be based on the index scores themselves instead of relative ranks. One reason is that since we only have two indices, it doesn't make sense to use ranks and lose information. Secondly, we want to be more precise in how much weight to assign.
...
```{r weighted_conf_mat, results='asis'}
...
Accuracy is defined as the number of correct clusterings out of the total number of clusterings. In our case, we consider the *truth* to be the reference clustering from TCGA.
...
worms.final <- worms.all %>%
  ...
  dplyr::select(-weight.1, -weight.2)
...
```{r worms_best}
plot(sw$data, col = c(2:3)[worms.final$`HC (S. Euc)`], xlab = "x", ylab = "y",
...
indices <- data.frame(
  ...
  DBI = dat %>%
    ...
    rank(ties.method = "min"),
  DI = rank(-apply(rings.all, 2, dunn, distance = dist(dat)),
            ties.method = "min"),
  RS = rank(-apply(rings.all, 2, index.S, d = dist(dat)),
            ties.method = "min"),
  CI = rank(apply(rings.all, 2, index.G3, d = dist(dat)),
            ties.method = "min"),
  GI = rank(-apply(rings.all, 2, index.G2, d = dist(dat)),
            ties.method = "min"),
  CHI = rank(-apply(rings.all, 2, index.G1, x = dat),
             ties.method = "min"),
  Conn = rank(apply(rings.all, 2, clValid::connectivity, distance = dist(dat)),
              ties.method = "min")) %>% 
  mutate(Top = apply(.[, -1], 1,
                     function(x) paste0(round(100 * sum(x %in% c(1, 2)) /
                                                ncol(.[, -1]), 2), "%"))) %>% 
...
rings.final <- rings.all %>% 
  as.data.frame %>% 
  mutate(Meta = as.factor(cutree(hclust(dist(consensusMatrix(rings.all)), method = "average"), 3)))

rings.matches <- rings.final %>% 
  apply(., 2, function(x) sum(x == true.class) / length(x)) %>% 
  sort(decreasing = T) %>% 
  data.frame(Accuracy = .)
...
```{r rings_best}
plot(dat, col = c(2:4)[rings.final$`HC (S. Euc)`], xlab = "x", ylab = "y",
...
The seeds dataset from [UCI](https://archive.ics.uci.edu/ml/datasets/seeds) has 3 classes of seeds with 70 observations each (n = 210). After removing features with low variability, we perform consensus clustering with only area, perimeter, and asymmetry coefficient remaining. Each feature is scaled to have mean of 0 and standard deviation of 1.
...
The table of ranked indices is shown below:
...
```{r seeds_meta_unweighted, results='asis'}
...
```{r seeds_meta_weighted, results='asis'}
...
# weighted meta-consensus matrix
