...
We can now compare the **accuracy** of all `length(ALG.NAMES) + 2` clustering results: `length(ALG.NAMES)` individual clustering algorithms plus the unweighted and weighted meta-consensus clustering algorithms.
...
```{r seeds_weighted, results='asis'}
PAC.CHI.only <- data.frame(Algorithms = ALG.NAMES, PAC , CHI) %>% 
    mutate(Weight.PAC = (1 - PAC) / sum((1 - PAC)),
           Weight.CHI = CHI / sum(CHI),
           Weight = (Weight.PAC + Weight.CHI) / 2) %>% 
    arrange(desc(Weight)) %>% 
...
weights <- PAC.CHI.only[order(match(PAC.CHI.only$Algorithms, colnames(seeds.all))), "Weight"]
weighted.meta.cm <- consensusMatrix(seeds.all, weights)

final.compare <- hclust(dist(weighted.meta.cm), method = "average") %>%
  cutree(3) %>%
  as.factor %>%
  set_names(true.class) %>%
  table(Meta = ., UCI = names(.)) %>%
